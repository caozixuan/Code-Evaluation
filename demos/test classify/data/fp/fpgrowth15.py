...
class FPGrowth():
    ...
    def __init__(self, dataset, min_sup=0.0,min_conf=0.0):
        ...
        self.support_data = {} # dic 频繁项集及其对于支持度
    ...
    def __fpgrowth(self, cpb, post_model):
        ...
        for header in headers.values():
            ...
            self.support_data[frozenset(rule)] =  header.count / data_num
    ...
    def generate_big_rules(self):
        ...
        sub_set_list = []
        L = []
        for item_and_sup in self.freq_itemsets:
            L.append(frozenset(item_and_sup[0]))
        print("L",L)
        for freq_set in L:
            for sub_set in sub_set_list:
                if sub_set.issubset(freq_set):
                    ...
                    if conf >= self.min_conf and big_rule not in big_rule_list:
